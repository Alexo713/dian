...
int ele;   //电梯所在楼层
int max = 4;   //电梯核载人数
int stz[10], st[10], endz[10], end[10], time[10];   //用户起始楼层、目标楼层与呼叫时间(后缀z表示暂存)
...
int main()
{
	...
	printf("请输入电梯所在楼层：\n");
	scanf("%d", &ele);
	printf("请输入各用户起始楼层、终止楼层与呼叫时间(输入T结束)：\n");
	int n1 = -1;    //n1统计用户总人数
	for (int i = 0; i < 10; i++) {
		char x = getchar();
		if (x == 'T') {
	...
	if (fir > 0) {
		for (int i = 0; i < 10; i++) {
			time[i] -= fir;   //若最早呼叫时间大于0，则将所有呼叫时间向前推
		}
	}
